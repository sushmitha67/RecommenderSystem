...
"~","!","@","#","$","%","^","&","*","(",")","]", "[", "}", "{", "<",">","?","\\", "/", ")", "(","without", "see", "unless", "due", "also", "must", "might", "like","will", "may", "can", "much", "every", "the", "in", "other", "this", "the", "many", "any",  "or", "in", "an", "is", "a", "about", "above", "after", "again", "against", "all", "am",  "and", "any", "aren't", "as", "at", "be", "because", "been", "before", "being", "below", "between", "both", "but", "by", "can't", "cannot", "could",
...
"will", "with", "would", "yet", "you", "your","a's","able","about","above","according","accordingly","across","actually","after","afterwards","again ","against ","ain't ","all ","allow","allows","almost","alone","along","already","also","although","always","looking","look","looks","necessary","nearly","anything"};

public static LinkedHashSet<String> wordsList = new LinkedHashSet<String>();
public static ArrayList<String> wordsList1 = new ArrayList<String>();
...
{
    wordsList1.add(words[i].toLowerCase());
}
...
if (wordsList1.contains(stopWordsofwordnet[j])) 
{
	//System.out.println("matched= "+stopWordsofwordnet[j]);
        //System.out.println("i,j= "+i+","+j);
        wordsList1.remove(stopWordsofwordnet[j]);     
}
   
}
}
    System.out.println("len"+wordsList1.size());
